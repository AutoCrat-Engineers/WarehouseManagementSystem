...
export async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    let listQuery = sortField
      ? db.from('items').select('*', { count: 'exact' }).order(sortField, { ascending: sortAsc })
      : db.from('items').select('*', { count: 'exact' }).order('created_at', { ascending: false });
    ...
    const [listResult, totalR, activeR, inactiveR] = await Promise.all([
      ...
      db.from('items').select('id', { count: 'exact', head: true }).eq('is_active', true),
      db.from('items').select('id', { count: 'exact', head: true }).eq('is_active', false),
    ]);
