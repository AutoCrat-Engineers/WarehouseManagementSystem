 ...
 *  - Stock Distribution:    vw_item_stock_distribution (for View modal — Multi Warehouse Stock tab)
 ...
 *  4. 4-tab View Modal: Item Details → Multi Warehouse Stock → Blanket Orders → Blanket Release
...
function DeleteConfirmModal({ isOpen, onClose, onConfirm, item }: DeleteConfirmModalProps) {
  ...
  return (
    <>
      ...
      {/* ── Secondary "Are you sure?" confirmation ── */}
    ...
    <div style={{ border: isExpanded ? '1.5px solid var(--enterprise-primary)' : '1px solid var(--enterprise-gray-200)', borderRadius: 'var(--border-radius-lg)', overflow: 'hidden', marginBottom: '10px', transition: 'all 0.25s cubic-bezier(0.4,0,0.2,1)', boxShadow: isExpanded ? '0 8px 24px rgba(30,58,138,0.12)' : '0 1px 3px rgba(0,0,0,0.04)', background: 'white' }}>
      ...
      >
        ...
        <div style={{ flex: 1, padding: '12px 20px', display: 'flex', flexDirection: 'column', justifyContent: 'center', background: 'var(--enterprise-gray-50)', minWidth: '180px' }}>
          <div style={{ marginBottom: '8px' }}><p style={upperLabelStyle}>Customer PO Number</p><p style={{ ...upperValueStyle, fontFamily: 'monospace', color: order.customer_po_number === '—' ? 'var(--enterprise-gray-400)' : 'var(--enterprise-gray-800)' }}>{order.customer_po_number}</p></div>
          <div><p style={upperLabelStyle}>Order Date</p><p style={{ ...upperValueStyle, color: 'var(--enterprise-gray-700)', display: 'flex', alignItems: 'center', gap: '6px' }}><Calendar size={12} style={{ color: 'var(--enterprise-gray-400)' }} />{order.order_date ? new Date(order.order_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}</p></div>
        </div>
      </div>
      {isExpanded && (
...
function ItemViewModal({ isOpen, onClose, item }: {
  ...
  const fetchBlanketOrders = async () => {
    ...
    try {
      ...
      const transformed: BlanketOrder[] = ordersData.map((row: ViewItemDetails, idx: number) => ({
        ...
        sap_doc_no: row.sap_doc_no || '—', master_serial_no: row.master_serial_no || '—', part_number: row.part_number || '—',
        customer_name: row.customer_name || '—', line_no: row.line_number ?? idx + 1, blanket_order_qty: row.blanket_quantity ?? row.item_quantity ?? 0,
        customer_po_number: row.customer_po_number || '—', order_date: row.order_date || '',
  ...
  /* Reusable detail field — clean label/value (no per-field icons) */
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Details of ${item.master_serial_no || 'MSN'}`} maxWidth="1000px">
      ...
      {/* ══════ TAB 1: ITEM DETAILS — New 6 Row Layout ══════ */}
      ...
      {/* ══════ TAB 2: MULTI WAREHOUSE STOCK ══════ */}
      <div style={tabContentStyle('multiWarehouseStock')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          {/* Item Summary Header — Part Number, UOM, Status */}
...
export function UnifiedItemMaster({ userRole, userPerms = {} }: UnifiedItemMasterProps) {
  ...
  // ── Item counts (lightweight server-side) ──
  ...
  // ── FETCH: Items (server-side filtered + paginated) ──
  ...
  // ── Refresh: reload data only (does NOT clear search or filters) ──
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {/* ═══ SUMMARY CARDS — Total Items, Active Items, Inactive Items ═══ */}
      <SummaryCardsGrid>
        <SummaryCard
          label="Total Items"
      ...
      {/* ═══ FILTER BAR — Search across MSN, Part Number, Description ═══ */}
      <SharedFilterBar>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ backgroundColor: 'var(--table-header-bg)', borderBottom: '2px solid var(--table-border)' }}>
                    ...
                    <th style={{ ...thStyle, minWidth: '120px', cursor: 'pointer', userSelect: 'none' }} onClick={() => handleSort('master_serial_no')} title={sortField === 'master_serial_no' ? (sortDirection === 'asc' ? 'Sorted A→Z · Click for Z→A' : 'Sorted Z→A · Click to clear') : 'Click to sort A→Z'}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', color: sortField === 'master_serial_no' ? 'var(--enterprise-primary)' : undefined }}>
                        MSN{sortField === 'master_serial_no' && <span style={{ fontSize: '10px', opacity: 0.8 }}>{sortDirection === 'asc' ? '▲' : '▼'}</span>}
                      </span>
                    </th>
                    <th style={{ ...thStyle, minWidth: '130px', cursor: 'pointer', userSelect: 'none' }} onClick={() => handleSort('part_number')} title={sortField === 'part_number' ? (sortDirection === 'asc' ? 'Sorted A→Z · Click for Z→A' : 'Sorted Z→A · Click to clear') : 'Click to sort A→Z'}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', color: sortField === 'part_number' ? 'var(--enterprise-primary)' : undefined }}>
                        Part Number{sortField === 'part_number' && <span style={{ fontSize: '10px', opacity: 0.8 }}>{sortDirection === 'asc' ? '▲' : '▼'}</span>}
                      </span>
                    </th>
